...
def replace_caption_by_index(captions):
  ...
  caption_text_list  = map(lambda x: x.strip(), caption_text_list)
  
  captionToIdx = {key: val for val, key in enumerate(caption_text_list)}

  count = 0
  for c in captions:
    c = c.strip()
    if c in captionToIdx:
      idx = captionToIdx.get(c)
...
if __name__ == '__main__':
  ...
  # coco_mini_val_dataset = set(list(coco_mini_val_dataset)[:5])
